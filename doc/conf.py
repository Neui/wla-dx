...
use_git_version = os.getenv('USE_GIT_VERSION', 'True') is ('true', 'on')
...
if version_info_h_file:
  with open(version_info_h_file, 'rt') as h_file:
    release = re.search('#define\s+VERSION_FULL_STRING\s+\"v?([^"]+)\"', h_file.read()).group(1)
elif use_git_version:
  proc = subprocess.Popen('git describe --dirty'.split(' '),
      cwd=repo_dir, stdout=subprocess.PIPE)
  stdout, stderr = proc.communicate()
  if proc.returncode != 0:
    raise subprocess.CalledProcessError("Couldn't git describe: %d" % proc.returncode)
  release = stdout.read()
else:
  try:
